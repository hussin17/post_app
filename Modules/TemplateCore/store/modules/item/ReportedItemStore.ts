...
defineStore(`reportedItemStore/${key}`,
 () => {
    ...
    async function addReportItem(loginUserId: string, holder: ReportItemHolder): Promise<PsResource<Product>> {

        loading.value = true;

        const responseData = await PsApiService.postReportItem<Product>(new Product(), loginUserId, holder.toMap());
