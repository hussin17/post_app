                        ...
                        <div class="grid w-full gap-6 sm:w-1/2">
                            <div>
                                ...
                                <div v-if="blog.cover[0]" class="flex items-end pt-4">
                                    <img v-lazy="{ src: $page.props.uploadUrl + '/' + blog.cover[0]?.img_path, loading: $page.props.sysImageUrl + '/loading_gif.gif', error: $page.props.sysImageUrl + '/default_photo.png' }"
                                        width="400" height="200" class="h-48 w-96" alt="blog cover" />
                                    <ps-button type="button" @click="replaceImageClicked(blog.cover[0].id)"
                            ...
                            <div>
                                <div v-for="(coreField, index) in coreFieldFilterSettings.filter((coreField) => coreField.original_field_name === 'link_type' && coreField.enable === 1 && coreField.is_delete === 0)"
                                    ...
                                    <select style="width: 100%; border: 1px solid #DDD;" v-model="form.link_type"
                                        type="text" :placeholder="$t(coreField.placeholder)">
