...
class ProductApiController extends Controller
{
    ...
    public function getComments(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if ($request->enable_last) {
                // Insert Comment
                $comments = DB::table('psx_item_comments')
                    ->join('users', 'psx_item_comments.user_id', '=', 'users.id')
                    ->where('item_id', $request->item_id)
                    ->offset($request->start ? $request->start : 0)
                    ->limit($request->limit)
                    ->orderBy('psx_item_comments.id', 'DESC')
                    ->get(['users.id', 'users.name', 'users.user_cover_photo', 'psx_item_comments.*']);
    ...
    public function add_Comment(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $response = DB::table('psx_item_comments')->insertGetId([
    ...
    {
        if ($request->comment && $request->id && $request->login_user_id) {
            DB::table('psx_item_comments')->where('id', $request->id)->where('user_id', $request->login_user_id)->update([
                'comment' => $request->comment
            ]);
            $res = [
                'success' => true
    ...
    {
        if ($request->id && $request->login_user_id) {
