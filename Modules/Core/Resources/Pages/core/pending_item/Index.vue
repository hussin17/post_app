<template>
    ...
    <ps-layout>
        ...
        <ps-table2 ::row="row" :search="search" :object="this.items" :colFilterOptions="colFilterOptions" :columns="columns"
            :sort_field="sort_field" :sort_order="sort_order" :globalSearchPlaceholder="$t('core__be_search_pending_item')"
            ...
            <template #tableRow="rowProps">
                <div class="flex flex-row" v-if="rowProps.field == 'accept'">
                    <ps-button :disabled="!rowProps.row.authorizations.update"
                        @click="[form.status = 'accept', handleStatusChange(rowProps.row.id)]" colors="bg-green-500 text-white"
                        hover="hover:outline-none hover:ring hover:ring-green-100"
                        focus="focus:outline-none focus:ring focus:ring-green-200">{{ $t('core__be_accept_lbl')
                        }}</ps-button>
                </div>
                <div class="flex flex-row" v-if="rowProps.field == 'reject'">
                    <ps-button :disabled="!rowProps.row.authorizations.update"
                        @click="[form.status = 'reject', handleStatusChange(rowProps.row.id)]" colors="bg-red-500 text-white"
                ...
                <span v-if="rowProps.field == itmConditionOfItem + '@@name'">
                    <ps-badge theme="text-red-500 bg-red-100" class="" v-if="rowProps.row[itmConditionOfItem + '@@name']">{{
