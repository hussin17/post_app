                    ...
                    <form @submit.prevent="handleSubmit(item.id)">
                        <div class="grid xxl:grid-cols-2 xl:grid-cols-2 lg:grid-cols-2 md:grid-cols-2 sm:grid-cols-1">
                            <div class="grid gap-6">
                                ...
                                <!-- item touch count -->
                                <div class="md:me-6 sm:me-0"
                                    v-for="(coreField, index) in coreFieldFilterSettings.filter((coreField) => coreField.original_field_name === 'item_touch_count' && coreField.enable === 1 && coreField.is_delete === 0)"
                                    :key="index">
                                    <ps-label class="text-base">{{ $t(coreField.label_name) }} <span
                                            v-if="coreField.mandatory = 1"
                                            class="font-medium text-red-800 ms-1">*</span></ps-label>
                                    <ps-input ref="item_touch_count" type="number" v-model:value="form.item_touch_count"
                                        :placeholder="$t(coreField.placeholder)" />
                                    <ps-label-caption v-if="coreField.mandatory == 1" textColor="text-red-500 "
                                        class="block mt-2">{{ errors.item_touch_count }}</ps-label-caption>
                                </div>
                                            ...
                                            <div class="w-full rounded-md shadow-xs bg-background dark:bg-backgroundDark">
                                                <div class="z-30 pt-2">
                                                    <div v-if="currencies.length == null">
                                                        <ps-label class="flex p-2" @click="route('currency.index')">{{
                                ...
                                <div class="md:me-6 sm:me-0"
                                    ...
                                    :key="index">
                                    <ps-label class="mb-1 text-base">{{ $t(coreField.label_name) }}</ps-label>
                                ...
                                <div>
                                    ...
                                    <!-- <div v-if="item.cover[0]" class="flex items-end pt-4">
                                        ...
                                        v-lazy=" { src: $page.props.uploadUrl + '/' + item.cover[0].img_path, loading: $page.props.sysImageUrl+'/loading_gif.gif', error: $page.props.sysImageUrl+'/default_photo.png' }"
                                         class="h-48 w-96" :alt="$t(core__be_item_photo)" />
                                        <ps-button type="button" @click="replaceImageClicked(item.cover[0].id,'item')" rounded="rounded-full" shadow="drop-shadow-2xl" class="mb-2 -ms-10" colors="bg-white text-primary-500 dark:bg-secondaryDark-black" border="border border-1 dark:border-secondary-700 border-secondary-300" padding="p-1.5" hover="" focus="">
                                            <ps-icon name="pencil-btn"  w="21" h="21" />
                                        </ps-button>
                                        <ps-image-icon-modal ref="ps_image_icon_modal" />
                                        <ps-action-modal ref="ps_action_modal" />
                                    ...
                                    <Dropzone @clicked="pushImage" @removeImage="removeImage" @caption="caption"
                                        :images="form.item_image" :edit_mode="1" :item_id="item.id"
                            ...
                            <div class="w-full ms-4">
                                ...
                                <div class="mb-6" v-else>
                                    <open-street-map-pin-picker :onChange="setCoordinates" :lat="parseFloat(form.lat)"
                                        :lng="parseFloat(form.lng)" class="h-[500px]"></open-street-map-pin-picker>
                                </div>
                                <!-- maps end -->
                        ...
                        <div class="flex flex-row justify-end mb-2.5">
                            ...
                            <ps-button v-if="paidItemProgressStatus != item.paid_status" type="button"
                                @click="handlePromote(item.id)" class="transition-all duration-300 ms-3 min-w-3xs"
        ...
        let form = useForm(
            {
                ...
                item_touch_count: props.item.item_touch_count,
