<template>
    ...
    <ps-layout>
        ...
        <ps-table2 :row="row" :search="search" :object="frontend_languages" :colFilterOptions="colFilterOptions"
            ...
            <template #button>
                    <ps-button v-if="can.createLanguage" @click="handleCreate()" rounded="rounded-lg" type="button" class="flex flex-wrap items-center"> <ps-icon name="plus" class="me-1 font-semibold" /> {{ $t('core__be_add_frontend_language') }}</ps-button>
            </template>
...
export default defineComponent({
       ...
       let visible = ref(false)
        ...
        function confirmDeleteClicked(id) {
            ps_danger_dialog.value.openModal(
               trans('core__be_delete_felanguage'),
                trans('core__be_delete_felanguage_info'),
            ...
            Inertia.get(route('fe_language.index'),
                {
                    ...
                    page: page ?? props.frontend_languages.meta.current_page,
                    row: row ?? props.frontend_languages.meta.per_page,
                    search: search.value,
                },
                {
                    preserveScroll: true,
    ...
    methods: {
        handleCreate() {
            this.$inertia.get(route("fe_language.create"));
        },
        handleEdit(id){
            this.$inertia.get(route('fe_language.edit',id));
        },
        handlePublish(row){
            this.$inertia.put(route('fe_language.statusChange',row.id), '', {
                onSuccess: () => {
            ...
            Inertia.post(route('fe_language.screenDisplayUiSetting.store'),
                {
                    ...
                    row: this.frontend_languages.per_page,
                    search: this.search.value,
                    current_page: this.frontend_languages.current_page
                },
                {
                    preserveScroll: true,
                    preserveState: true,
