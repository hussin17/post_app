<template>
    ...
    <ps-layout>
        ...
        <!-- data table start -->
        <ps-table2 :row="row" :search="search" :object="this.fields" :colFilterOptions="colFilterOptions" :columns="columns"
            ...
            <!-- add new field start -->
            <!-- <template #button>
                <ps-button v-if="can.createLanguageString"  @click="handleCreate()"  rounded="rounded-lg" type="button" class="flex flex-wrap items-center"> <ps-icon name="plus" class="font-semibold me-2" />{{$t('core__be_add_language')}}</ps-button>
            </template>
            ...
            <template #tableActionRow="rowProps">
                ...
                <div class="flex flex-row mb-2" v-if="rowProps.field == 'action'">
                    ...
                    <ps-button :disabled="rowProps.row.authorizations.delete ? permission_for_delete == '0' : true"
                        ...
                        focus="focus:outline-none focus:ring focus:ring-red-200"> <ps-icon
                            theme="text-white dark:text-primary-900" name="trash" w="16" h="16" /> </ps-button>
            ...
            <template #tableRow="rowProps">
                ...
                <div class="flex" v-if="rowProps.field == 'name'">
                    <div class="">
                        <ps-button :disabled="rowProps.row.authorizations.update ? false : true"
                            @click="languageClicked(rowProps.row.nameKey)" class="me-4" colors="bg-primary-500 text-white"
                ...
                <div class="flex" v-if="rowProps.field == 'placeholder'">
                    <div class="">
                        <ps-button :disabled="rowProps.row.authorizations.update ? false : true"
                            @click="languageClicked(rowProps.row.placeholderKey)" class="me-4"
                            colors="bg-primary-500 text-white" padding="p-1.5"
                ...
                <div v-if="rowProps.field == 'ui_type_id'">
                    <ps-label class="font-normal">{{ rowProps.row.ui_type_id ? $t(rowProps.row.ui_type_id.name) : $t('N.A')
                    }}</ps-label>
                </div>

                <div v-if="rowProps.field == 'show_in_table'">
                    <ps-button :disabled="rowProps.row.authorizations.update ? false : true"
                        @click="eyeIconClicked(rowProps.row.id, rowProps.row.is_core_field, rowProps.row.is_include_in_hideshow, rowProps.row.is_show, rowProps.row.is_show_in_filter)"
                        class="me-4" colors="bg-primary-500 text-white" padding="p-1.5"
                        hover="hover:outline-none hover:ring hover:ring-green-100"
                        focus="focus:outline-none focus:ring focus:ring-green-200">
                ...
                <div v-if="rowProps.field == 'enable'">
                    <ps-toggle
                        :disabled="rowProps.row.authorizations.update ? rowProps.row.permission_for_enable_disable == 1 ? false : true : true"
                        :selectedValue="rowProps.row.enable == 1 ? true : false"
                        @click="handlePublish(rowProps.row.id, rowProps.row.authorizations.update && rowProps.row.permission_for_enable_disable == '1', rowProps.row.is_core_field)"
                        toggleOnTheme="bg-primary-500 border-primary-500 "></ps-toggle>
                </div>

                <div v-if="rowProps.field == 'is_show_sorting'">
                    <ps-toggle :disabled="rowProps.row.authorizations.update ? false : true"
                        :selectedValue="rowProps.row.is_show_sorting == '1' ? true : false"
                        @click="handleIsShowSorting(rowProps.row.id, rowProps.row.authorizations.update, rowProps.row.is_core_field)"
                        toggleOnTheme="bg-primary-500 border-primary-500 "></ps-toggle>
                </div>
                ...
                <ps-button
                    :disabled="rowProps.row.authorizations.update ? rowProps.row.permission_for_mandatory == '1' ? false : true : true"
                    v-if="rowProps.field == 'mandatory'"
                    @click="handleMandatory(rowProps.row.id, rowProps.row.permission_for_mandatory, rowProps.row.is_core_field)"
                    :padding="[rowProps.row.mandatory == '1' ? 'py-2 px-2' : 'py-2 px-4']"
                    :colors="[rowProps.row.mandatory == '1' ? 'bg-red-400  dark:bg-red-200 text-white dark:text-red-800' : 'bg-green-400', ' dark:bg-green-200 text-white dark:text-green-800']">
                    {{ rowProps.row.mandatory == '1' ? $t('core__be_btn_mandatory') : $t('core__be_btn_optional') }}
                </ps-button>

                <ps-button v-if="rowProps.field == 'attribute'"
                    :disabled="rowProps.row.authorizations.update ? rowProps.row.is_core_field == '1' ? true : isIncludehaveAttrUiTypes(rowProps.row.ui_type_id?.core_keys_id) : true"
                    @click="handleCustomDetail(rowProps.row.core_keys_id, rowProps.row.is_core_field, rowProps.row.ui_type_id.core_keys_id)"
                    class="me-4"
                    :colors="isEnableAttribute(rowProps.row) ? 'bg-green-400 text-white' : 'bg-primary-100 text-white'"
                    padding="p-1.5" hover="hover:outline-none hover:ring hover:ring-green-100"
                    focus="focus:outline-none focus:ring focus:ring-green-200">
                    <ps-icon
                        :theme="isEnableAttribute(rowProps.row) ? 'text-white dark:text-primary-900' : 'text-primary-300 dark:text-primary-900'"
                        name="editPencil" w="16" h="16" />
                </ps-button>
        ...
        <ps-dialog-with-input2 ref="ps_dialog_with_input2">
            <template #body>
                <div class="w-full mt-2 text-start">
                    ...
                    <ps-input type="number" v-model:value.number="columnOrderingForm.ordering"
                        :placeholder="$t('column_order_placeholder')" />
                </div>
            </template>
