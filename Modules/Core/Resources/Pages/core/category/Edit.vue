                ...
                <div class="px-4 pt-6 dark:bg-backgroundDark">
                    <form @submit.prevent="handleSubmit(this.category.id)">
                        <div class="grid w-full gap-6 sm:w-1/2">
                            ...
                            <div>
                                <ps-label class="mb-2 text-base">{{ $t('core__category_photo_label') }}<span
                                        class="font-medium text-red-800 ms-1">*</span>
                                </ps-label>
                                <ps-label-title-3 v-if="!category.cover"> {{ $t('core__be_recommended_size_400_200') }}
                                </ps-label-title-3>
                                <div v-if="category.cover" class="flex items-end pt-4">
                                    <img v-lazy="{ src: $page.props.uploadUrl + '/' + category.cover.img_path, loading: $page.props.sysImageUrl + '/loading_gif.gif', error: $page.props.sysImageUrl + '/default_photo.png' }"
                                        class="h-48 w-96" alt="category cover" />
                            ...
                            <div>
                                <ps-label class="mb-2 text-base">{{ $t('core__category_icon_label') }}<span
                                        class="font-medium text-red-800 ms-1">*</span>
                                </ps-label>
                                <ps-label-title-3 v-if="!category.icon"> {{ $t('core__be_recommended_size_200_200') }}
                                </ps-label-title-3>
                                <div v-if="category.icon" class="flex items-end pt-4">
                                    <img v-lazy="{ src: $page.props.uploadUrl + '/' + category.icon.img_path, loading: $page.props.sysImageUrl + '/loading_gif.gif', error: $page.props.sysImageUrl + '/default_photo.png' }"
                                        class="w-48 h-48" alt="category icon" />
                                    <ps-button type="button" @click="replaceIconClicked(category.icon.id)"
                                        rounded="rounded-full" shadow="drop-shadow-2xl" class="mb-2 -ms-10"
                                        colors="bg-white text-primary-500 dark:bg-secondaryDark-black"
                                        border="border border-1 dark:border-secondary-700 border-secondary-300"
                                        padding="p-1.5" hover="" focus="">
