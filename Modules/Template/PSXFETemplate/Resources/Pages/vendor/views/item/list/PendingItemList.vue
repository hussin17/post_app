<template>
    <Head :title="$t('pending_item_list__pending_item_list')"/>
        ...
        <template #content>
            <div class="mt-32">
                ...
                <div class="lg:flex md:flex lg:justify-between md:justify-between mt-6">
                    <ps-label-header-4 class="font-medium"> {{ $t("pending_item_list__pending_item_list") }}
                    </ps-label-header-4>
                ...
                <div class="flex flex-col mt-6">
                    ...
                    <div class='flex flex-row mb-8 '>
                        <div class="w-full flex flex-col">
                            <!-- paid & promote ads -->
                            <div v-if="itempendingProvider.itemList?.data != null">
                                <div class="grid grid-cols-4 sm:grid-cols-4 md:grid-cols-9 lg:grid-cols-12 md:gap-6 gap-4 ">
                                    ...
                                    <div class="w-full col-span-4 sm:col-span-2 md:col-span-3 lg:col-span-4 xl:col-span-3"
                                        v-for="item in itempendingProvider.itemList.data" :key="item.id">


                                            <item-horizontal-item :item="item" storeName="pending" />
                    ...
                    <ps-button v-if="itempendingProvider.loading.value == false" class="w-60 mx-auto mt-8" @click="loadMore"
                        :class="itempendingProvider.isNoMoreRecord.value ? 'hidden' : ''"> {{ $t("list__load_more") }}
                    </ps-button>
...
export default {
    ...
    setup(props) {
        ...
        itempendingProvider.limit = props.mobileSetting?.default_loading_limit ?? 12;
        ...
        itempendingProvider.resetProductList(loginUserId, holder);
    ...
    computed: {
        breadcrumb() {
            return [
                ...
                {
                    label: trans('pending_item_list__pending_item_list'),
