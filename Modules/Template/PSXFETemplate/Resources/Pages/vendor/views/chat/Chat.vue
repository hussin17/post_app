        ...
        <template #content>
            <div class="relative flex flex-col " style="height:90vh" >
                ...
                <div class='lg:mt-32 mt-28 h-48 flex flex-col' >
                    ...
                    <!-- breadcrumb end -->
                    <div class='flex flex-wrap items-center mb-2 mt-2 p-2 bg-gray-50  rounded dark:bg-feBackgroundDark'>
                        ...
                        <ps-route-link :to="{
                            ...
                            <div class="flex flex-row content-end items-center cursor-pointer">
                                <img alt="Placeholder"  width="300px" height="200px" class="w-12 h-12 object-cover bg-white rounded-full me-2"
                        ...
                        <div class="grow">
                            ...
                            <div v-if="isOtherUserOnline" class="flex flex-row items-center">
                                <ps-icon name='dot' w="10" h="10" class=' text-green-300 me-2'> </ps-icon>
                                <ps-label> {{ $t("chat__online") }} </ps-label>
                            </div>
                            <div v-else class="flex flex-row items-center">
                                <ps-icon name='dot' w="10" h="10" class=' text-gray-300 me-2'> </ps-icon>
                        ...
                        <!-- <div v-if="chatFlag == PsConst.CHAT_FROM_SELLER && showOffer" class="flex flex-col content-end items-center">
                            <div  >
                                <ps-label class="select-none px-4 py-3 my-auto bg-fePrimary-500 dark:bg-fePrimaryDark-accent text-median font-medium text-textLight dark:text-textDare rounded-xl lg:rounded-2xl  cursor-pointer"
                    ...
                    <div v-if="dataReady" class='flex sm:flex-row justify-between bg-gray-50 p-2 rounded dark:bg-feBackgroundDark'>
                        <ps-route-link :to="{ name: 'fe_item_detail', query: { item_id: itemId }}">
                            <div class='flex flex-row cursor-pointer'>
                                <img alt="Placeholder" class="w-16 h-16 bg-white rounded-md"
                        ...
                        <div class="chat" v-if='chat.message != "" && chat.message != null '>
                            ...
                            <div v-else-if="chat.type == PsConst.CHAT_TYPE_TEXT">
                                ...
                                <div v-if="chat.offerStatus == PsConst.CHAT_STATUS_OFFER">
                                    ...
                                    <div v-if='chat.sendByUserId == loginUserId' class='w-full flex justify-end ps-1/3 flex-row items-end'>
                                        <ps-label class="me-2 mb-1 text-xs font-normal" textColor="text-feBackgroundDark dark:text-feTextLight"> {{chat.timeString}} </ps-label>
                                        <span class="px-2 py-4 mt-2 mb-1 rounded-md inline-block rounded-bl-none bg-blue-500 ">
                                            <ps-label class="text-center " textColor="text-feBackgroundDark" > {{ $t("chat__send_offer") }} <br/>{{ itemPrice == '0' ? $t("item_price__free") : chat.message}} </ps-label>
                                        </span>
                                    </div>
                                    ...
                                    <div v-else class="pe-1/3 flex flex-col">
                                        <div  class=" flex flex-row items-end">
                                            <span class="px-2 py-4 mt-2 mb-1 rounded-md inline-block rounded-br-none bg-blue-500 ">
                                                <ps-label  textColor="text-feTextLight" > {{ $t("chat__receive_offer") }} <br/>{{ itemPrice == '0' ? $t("item_price__free") : chat.message}}</ps-label>
                                            </span>
                                            <br class="sm:hidden">
                                            <ps-label class="ms-2 mb-1 text-xs font-normal" textColor="text-feBackgroundDark dark:text-feTextLight"> {{chat.timeString}} </ps-label>
                                        </div>
                                    </div>
                                ...
                                <div v-else-if="chat.offerStatus == PsConst.CHAT_STATUS_REJECT">
                                    ...
                                    <div v-if='chat.sendByUserId == loginUserId' class='w-full flex justify-end ps-1/3 flex-row items-end'>
                                        <ps-label class="me-2 mb-1 text-xs font-normal" textColor="text-feBackgroundDark dark:text-feTextLight"> {{chat.timeString}} </ps-label>
                                        <span class="px-4 py-2 mt-2 mb-1 rounded-lg inline-block rounded-bl-none bg-red-500 text-white">
                                            <ps-label class="text-center" textColor="text-feBackgroundDark dark:text-feTextLight" > {{ $t("chat__offer_rejected_item") }}</ps-label>
                                        </span>
                                    </div>
                                    ...
                                    <div v-else class="pe-1/3 flex flex-col">
                                        <div  class=" flex flex-row items-end">
                                            <span class="px-4 py-2 mt-2 mb-1 rounded-lg inline-block rounded-br-none bg-red-500 ">
                                                <ps-label  textColor="text-feTextLight">  {{ $t("chat__offer_rejected_item") }}</ps-label>
                                            </span>
                                            <ps-label class="ms-2 mb-1 text-xs font-normal" textColor="text-feBackgroundDark dark:text-feTextLight"> {{chat.timeString}} </ps-label>
                                        </div>
                                    </div>
                                    ...
                                    <div v-else>
                                        ...
                                        <div v-if='chat.sendByUserId == loginUserId' class="w-full flex flex-col justify-end items-end">
                                            <div  class=' flex flex-row items-end mb-2'>
                                                <ps-label class="me-2 mb-1 text-xs font-normal" textColor="text-feBackgroundDark dark:text-gray-400"> {{chat.timeString}} </ps-label>
                                                <span class="px-2 py-4 rounded-md inline-block rounded-bl-none bg-green-500">
                                                    <ps-label class="text-center  " textColor="text-feTextLight"  >  {{ $t("chat__offer_accepted_item") }}</ps-label>
                                                </span>
                                            </div>
                                            ...
                                            <div  class=' flex flex-row items-end mb-2' v-if="chat.isUserBought">
                                                <ps-label class="me-2 mb-1 text-xs font-normal" textColor="text-feBackgroundDark dark:text-gray-400"> {{chat.timeString}} </ps-label>
                                                <span class="px-2 py-4 rounded-md inline-block rounded-bl-none bg-blue-500">
                                                    <ps-label class="text-center  " textColor="text-feTextLight"  >  {{ $t("chat__offer_item_pickup") }}</ps-label>
                                                </span>
                                            </div>
                                            <!-- Item pick up message -->
                                             ...
                                             <div  class=' flex flex-row items-end mb-2' v-if="isSoldOut == '1'">
                                                <ps-label class="me-2 mb-1 text-xs font-normal" textColor="text-feBackgroundDark dark:text-gray-400"> {{chat.timeString}} </ps-label>
                                                <span class="px-2 py-4 rounded-md inline-block rounded-bl-none bg-red-500">
                                                    <ps-label class="text-center  " textColor="text-feTextLight"  >  {{ $t("chat__offer_item_soldout") }}</ps-label>
                                                </span>
                                            </div>
                                            ...
                                            <div v-if="chat.offerStatus == PsConst.CHAT_STATUS_ACCEPT && chat.isUserBought == false" class="flex justify-center w-full mt-5">
                                                <!-- <ps-label @click="isUserBought(chat)" class="p-2 flex flex-shrink-0 bg-fePrimary-500 dark:bg-fePrimaryDark-accent rounded-lg cursor-pointer " textColor="text-textLight dark:textDark "> {{ $t("chat__user_has_a_pickup") }} </ps-label> -->
                                                <ps-button @click="userPickup(chat)" colors="bg-white text-fePrimary-500 dark:bg-gray-800 dark:text-gray-200 hover:text-white" focus="focus:text-white focus:bg-fePrimary-500" border="border border-gray-300 dark:border-gray-500">
                                        ...
                                        <div v-else class="pe-1/3 flex flex-col">
                                            ...
                                            <!-- Item accept Message -->
                                            <div  class=" flex flex-row items-end mb-2">
                                                <span class="px-2 py-4 rounded-md inline-block rounded-br-none bg-green-500 ">
                                                    <ps-label class="  " textColor="text-feTextLight"  > {{ $t("chat__offer_accepted_item") }}</ps-label>
                                                </span>
                                                <ps-label class="ms-2 mb-1 text-xs font-normal" textColor="text-feBackgroundDark dark:text-feTextLight"> {{chat.timeString}} </ps-label>
                                            </div>
                                            <!-- Item accept message -->
                                            ...
                                            <!-- Item pickup message -->
                                            <div  class=' flex flex-row items-end mb-2' v-if="chat.isUserBought">
                                                <span class="px-2 py-4 rounded-md inline-block rounded-br-none bg-blue-500">
                                                    <ps-label class="text-center  " textColor="text-feTextLight"  >  {{ $t("chat__offer_item_pickup") }}</ps-label>
                                                </span>
                                                <ps-label class="ms-2 mb-1 text-xs font-normal" textColor="text-feBackgroundDark dark:text-gray-400"> {{chat.timeString}} </ps-label>
                                            </div>
                                            <!-- Item pickup message -->

                                            <!-- Item soldout message -->
                                            <div  class=' flex flex-row items-end mb-2' v-if="isSoldOut == '1'">
                                                <span class="px-2 py-4 rounded-md inline-block rounded-br-none bg-red-500">
                                                    <ps-label class="text-center  " textColor="text-feTextLight"  >  {{ $t("chat__offer_item_soldout") }}</ps-label>
                                                </span>
                                                <ps-label class="ms-2 mb-1 text-xs font-normal" textColor="text-feBackgroundDark dark:text-gray-400"> {{chat.timeString}} </ps-label>
                                            </div>
                                ...
                                <div v-else>
                                    <!-- Send by me UI -->
                                    <div v-if='chat.sendByUserId == loginUserId' class='w-full flex justify-end ps-1/3 flex-row items-end'>
                                        <!-- <ps-label-caption-2 @click="chatDelete(chat.id)" class="me-2 mb-2 del cursor-pointer bg-red-500 p-1 rounded text-white"> Delete </ps-label-caption-2> -->
                                        <div class="">
                                            <ps-label-caption-2 @click="showDeleteDropDown(chat.id)" :class="deleteChatId == chat.id ? '' : 'del'" class="cursor-pointer rounded float-right" textColor="text-feBackgroundDark dark:text-gray-400">
                                                <ps-icon name="verticalThreeDot" class="text-feBackgroundDark"></ps-icon>
                                            </ps-label-caption-2>

                                            <div class="relative">
                                                <div @click="deleteConfirm(chat)" class="bg-white shadow-sm absolute top-4 right-2 px-5  py-1 rounded border transform cursor-pointer text-sm dark:bg-feBackgroundDark dark:text-feTextLight" v-if="deleteChatId == chat.id">
                                        ...
                                        <span class="p-4 mt-2 mb-1 rounded-md inline-block rounded-bl-none bg-feSecondary-100 dark:bg-feSecondaryDark-black">
                                    ...
                                    <div v-else class="pe-1/3 flex flex-row items-end">
                                        <span class="p-4 mt-2 mb-1 rounded-md inline-block rounded-br-none  bg-fePrimary-500">
                                            <ps-label textColor="text-feTextLight" class="text-sm"> {{chat.message}} </ps-label>
                                        </span>
                                        <ps-label class="ms-2 mb-1 text-xs font-normal" textColor="text-feBackgroundDark dark:text-gray-400"> {{chat.timeString}} </ps-label>
                                    </div>
                                </div>
                            ...
                            <div v-else-if="chat.type == PsConst.CHAT_TYPE_BOUGHT">
                                ...
                                <div  class="mt-5 mb-2 grid grid-cols-4 gap-x-4 sm:flex sm:flex-row sm:justify-center  rtl:space-x-reverse sm:space-x-3">
                                    <!-- <div  class='flex  flex-row '>
                                        ...
                                        <span class="px-4 py-2 mt-2 mb-1 rounded-lg inline-block bg-fePrimary-000 dark:bg-fePrimaryDark-black ">
                                            <ps-label class="text-center " > {{ $t("chat__item_bought") }} <br/>{{itemPrice == '0' ? $t("item_price__free") : chat.message}} </ps-label>
                                        </span>
                                    </div> -->
                                    <div class="col-span-2">
                                        <ps-button @click="clickGiveReview()" colors="bg-white text-fePrimary-500 dark:bg-gray-800 dark:text-gray-200 hover:text-white" border="border border-gray-300 dark:border-gray-500">
                                    ...
                                    <div v-if='chat.sendByUserId == loginUserId && chat.offerStatus == PsConst.CHAT_STATUS_IS_USER_BOUGHT' class="col-span-2" >
                                        <!-- <ps-label @click="markAsSoldClick(chat)" class="p-2 flex flex-shrink-0 bg-fePrimary-500 dark:bg-fePrimaryDark-accent cursor-pointer rounded-lg" textColor="text-textLight dark:text-textDark"> {{ $t("chat__mark_as_sold") }} </ps-label> -->
                                        <ps-button @click="markAsSoldClick(chat)" class="w-full" colors="bg-fePrimary-500 text-white dark:bg-gray-800 dark:text-gray-200 hover:text-white" border="border border-gray-300 dark:border-gray-500">
                            ...
                            <div v-else-if="chat.type == PsConst.CHAT_TYPE_SOLD">
                                ...
                                <div  class="w-full mt-5 flex justify-center items-center">
                                    ...
                                    <div >
                                        <ps-button @click="clickGiveReview()" class=" h-9" colors="bg-white text-fePrimary-500 dark:bg-gray-800 dark:text-gray-200 hover:text-white" border="border border-gray-300 dark:border-gray-500">
                                            {{ $t("chat__leave_a_review") }}
                                        </ps-button>
                                    </div>
                            ...
                            <div v-else-if="chat.type == PsConst.CHAT_TYPE_OFFER">
                                ...
                                <div v-else class="flex flex-col">
                                    <div  class="flex flex-row items-end">
                                        <span class="px-2 py-4 mt-2 mb-1 rounded-md block rounded-br-none bg-blue-500 ">
                                            <ps-label class=" dark:text-fePrimaryDark-black" textColor="text-feTextLight" > {{ $t("chat__receive_offer") }} <br/>{{itemPrice == '0' ? $t("item_price__free") : chat.message}}</ps-label>
                                        </span>
                                        <ps-label class="ms-2 mb-2 text-xs font-normal" textColor="text-feBackgroundDark dark:text-feTextLight" > {{chat.timeString}} </ps-label>
                                    </div>
                                    <div v-if="chat.offerStatus == PsConst.CHAT_STATUS_OFFER" class="mt-5 mb-2 grid grid-cols-4 gap-4 sm:flex sm:flex-row sm:justify-center  rtl:space-x-reverse sm:space-x-4">
                                        ...
                                        <ps-button @click="acceptOffer(chat)" class="col-span-2 sm:w-[140px] h-9" textSize="font-normal text-xxs lg:text-sm" colors="bg-green-600 cursor-pointer text-feTextLight" rounded="rounded" hover="hover:bg-green-700" focus="focus:bg-green-700"> {{ $t("chat__accept") }}</ps-button>
                                        <ps-button @click="rejectOffer(chat)" class="col-span-2 sm:w-[140px] h-9" textSize="font-normal text-xxs lg:text-sm" colors="text-fePrimary-500 dark:bg-gray-800 dark:text-gray-200 hover:text-white" focus="focus:text-white focus:bg-fePrimary-500" border="border border-gray-300 dark:border-gray-500" rounded="rounded"> {{ $t("chat__reject") }} </ps-button>
                                    </div>
                                </div>
                            ...
                            <div v-else-if="chat.type == PsConst.CHAT_TYPE_IMAGE">
                                ...
                                <div v-if='chat.sendByUserId == loginUserId' class='w-full flex justify-end ps-1/3 flex-row items-end h-48 mt-4'>
                                    <div class="relative">
                                        ...
                                        <div @click="deleteConfirm(chat,chat.message)" class="bg-white shadow-sm absolute top-4 right-2 p-1 rounded border cursor-pointer transform" v-if="deleteChatId == chat.id">
                                            {{ $t('chat__delete_messages') }}
                                        </div>
                ...
                <div class="mt-2 mb-0 sm:mb-0 absolute inset-y-full w-full">
                    <div class="relative flex pt-1 w-full">
                        ...
                        <div class="grow relative">
                            ...
                            theme="form-input dark:bg-fePrimaryDark-black text-feSecondary-500 dark:text-feSecondaryDark-white"/>
