                ...
                <div class=" ">
                    <div class=" flex flex-row rtl:space-x-reverse space-x-2 sm:space-x-0 space-y-0 sm:space-y-2 sm:flex-col" id="sellerbtn" v-if="!isSellerFocus" :disabled="true">
                        <ps-button class="w-full" @click="buyerClicked" >
                            ...
                            <span class="rounded-full w-8 float-right bg-red-500 text-feTextLight">{{ userunreadmsgStore.unreadCount.data?.sellerUnreadCount}}</span>
                        </ps-button>

                        <ps-button class="w-full" colors="bg-white dark:bg-gray-800 dark:text-gray-200 hover:text-white" border="border border-gray-300 dark:border-gray-500" rounded="rounded" @click="sellerClicked">
                            <span class="me-2 md:me-4">{{ $t("chat_list__from_seller") }}</span>
                            <span class="rounded-full w-8 float-right bg-red-500 text-feTextLight">{{ userunreadmsgStore.unreadCount.data?.buyerUnreadCount}}</span>
                        </ps-button>
                    </div>
                    ...
                    <div class="flex flex-row rtl:space-x-reverse space-x-2 sm:space-x-0 space-y-0 sm:space-y-2 sm:flex-col" id="buyerbtn"  v-else :disabled="true">
                        <ps-button class="w-full" colors="bg-white dark:bg-gray-800 dark:text-gray-200 hover:text-white" border="border border-gray-300 dark:border-gray-500" rounded="rounded" @click="buyerClicked" >
                            <span class="me-2 md:me-4">{{ $t("chat_list__from_buyer") }}</span>
                            <span class="rounded-full w-8 float-right bg-red-500 text-feTextLight">{{ userunreadmsgStore.unreadCount.data?.sellerUnreadCount}}</span>
                        </ps-button>
