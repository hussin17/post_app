    ...
    <div class="w-full lg:rounded-lg sm:rounded-xl rounded-2xl sm:flex mt-4 sm:mt-0 gap-5">
        <div class="flex gap-x-5 flex-col md:flex-row gap-y-2">
                ...
                <ps-dropdown align="left" class='w-full md:w-52 h-full lg:rounded-2xl rounded' @onClick="loadSubLocation()">
                    ...
                    <template #list >
                        ...
                        <div class="pt-2 z-30">
                            ...
                            <div v-else class="w-56 flex py-4 px-2 hover:bg-fePrimary-000 dark:hover:bg-fePrimaryDark-black cursor-pointer items-center"  @click="categoryFilterClicked('',$t('item_list__all'))" >
                                <ps-label class="ms-2" :class="categoryId == '' ? 'font-bold text-fePrimary-700' : ''"  > All </ps-label>
                            </div>
                            <div v-for="selectData in categoryStore.itemList.data" :key="selectData.catId" class="w-56 flex py-4 px-2 hover:bg-fePrimary-000 dark:hover:bg-fePrimaryDark-black cursor-pointer items-center"  @click="categoryFilterClicked(selectData.catId, selectData.catName)" >
                ...
                <ps-dropdown :id="locationId ?? 'locaitonid'" align="left" class='w-full md:w-52 h-full lg:rounded-2xl rounded' @onClick="loadLocationList" >
                    ...
                    <template #filter >
                        <div>
                            <ps-input-with-left-icon rounded="rounded-t-lg" theme="ring-fePrimary-200 dark:ring-fePrimaryDark-grey focus:outline-none focus:ring focus:border-fePrimary-200 border-secondary-300 lg:rounded rounded-x bg-feSecondary-200 dark:bg-fePrimaryDark-black text-feSecondary-500 dark:text-feSecondaryDark-white" height="h-10" @keyup.enter="filterKeywordUpate(itemLocationStore.filterKeyword)"  v-model:value="itemLocationStore.filterKeyword" v-bind:placeholder= "$t('search_location')" >
                                <template #icon>
                ...
                <div class="flex justify-center">
                    <ps-button v-bind:title="$t('search_for_large_screem__search')" class="px-8 py-2 md:px-5 lg:px-8" rounded="rounded" theme="bg-fePrimary-500" @click="searchClicked()">
                        <ps-label class="text-sm font-medium" textColor="text-feTextLight dark:text-feSecondary-000"> {{ $t("search_for_large_screem__search") }} </ps-label>
                    </ps-button>
                </div>
