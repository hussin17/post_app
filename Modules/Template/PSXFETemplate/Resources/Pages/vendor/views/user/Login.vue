    ...
    <div>
        <div class=" sm:mt-32 lg:mt-36 mt-28  flex flex-col">
            ...
            <div class=" content-center mx-auto w-96"  >
                <ps-label class="content-center  text-general-200 dark:text-feSecondaryDark-white  px-4 py-4 rounded-md" v-if="userProvider.userResource.message != ''"> {{ userProvider.userResource.message }} </ps-label>
            </div>
            ...
            <div class="w-55 lg:w-80 flex flex-col mx-auto rounded-lg">
                ...
                <div class="mt-4 lg:mt-5 flex flex-col items-center">
                    <ps-label textColor="text-feSecondary-400 dark:text-feSecondaryDark-grey" class='cursor-pointer font-light text-xs lg:text-sm' @click='openForgotPassword' > {{ $t("login__forgot_password") }} </ps-label>
                    <ps-label textColor="text-feSecondary-400 dark:text-feSecondaryDark-grey" class='cursor-pointer mt-3 font-light text-xs lg:text-sm' @click='openRegister' > {{ $t("login__new_here") }} <span class="font-medium"> {{ $t("login__sign_up") }}</span> </ps-label>
                </div>
                ...
                <ps-button textSize="text-xs lg:text-base" class="mt-3 lg:mt-4 py-3 flex justify-center" @click="phoneloginclicked">
                    <font-awesome-icon :icon="['fas', 'phone']" class="text-feTextLight dark:text-feTextDark text-xl me-2" />
                    {{ $t("login__login_with_phone") }}
                </ps-button>

                <ps-button textSize="text-xs lg:text-base" class="mt-3 lg:mt-4 py-3 flex justify-center" theme="bg-blue-600 hover:bg-blue-700 text-feTextLight px-4 py-2" @click="facebookloginclicked">
                    <font-awesome-icon :icon="['fab', 'facebook-f']" class="text-feTextLight text-xl me-2"  />
                    {{ $t("login__login_with_facebook") }}
                </ps-button>
                <ps-button textSize="text-xs lg:text-base" class="mt-3 lg:mt-4 py-3 flex justify-center" theme="bg-yellow-600 hover:bg-yellow-700 text-feTextLight px-4 py-2"  id="signinBtn" @click="googleloginclicked">
                    <font-awesome-icon :icon="['fab', 'google']" class="text-feTextLight text-xl me-2"  />
                    {{ $t("login__login_with_google") }}
                </ps-button>
                <ps-button textSize="text-xs lg:text-base" class="mt-3 lg:mt-4 py-3 flex justify-center mb-5" theme="bg-gray-900 hover:bg-gray-700 text-feTextLight px-4 py-2"  @click="appleloginclicked">
                    <font-awesome-icon :icon="['fab', 'apple']" class="text-feTextLight text-xl me-2"  />
