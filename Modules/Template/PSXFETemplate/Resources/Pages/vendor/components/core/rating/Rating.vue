<template>
  <div class="flex items-center">
      <div v-for="star in maxStars" :class="{ 'active': star <= stars }" :key="star" @click="rate(star)" class="star me-0.5">
          <font-awesome-icon :icon="['fas', 'star']" :class="star <= grade ? 'text-yellow-500 ' : 'hidden ' " size="sm" />
          <font-awesome-icon :icon="['far', 'star']" :class="star <= grade ? 'hidden ' : 'text-yellow-500 ' " size="sm" />
      </div>
  </div>
</template>
