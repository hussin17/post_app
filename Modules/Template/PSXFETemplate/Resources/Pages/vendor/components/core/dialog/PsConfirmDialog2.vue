    ...
    <ps-modal ref="psmodal" maxWidth="472px" :isClickOut='false' theme="px-5 py-6 rounded-md" class=' z-20' :visibleLine="false">
        <template #title>
            <div class="w-full flex justify-between">
                ...
                <ps-icon name="close" class="text-gray-400 hover:cursor-pointer" @click.prevent="actionClicked('no')"></ps-icon>
            </div>
        </template>
        <template #body>
            <div class="w-full">
        ...
        <template #footer>
            <div class=" flex flex-row justify-end  rtl:space-x-reverse space-x-5">
                <!-- <ps-button @click="actionClicked('no')" textSize="text-xxs lg:text-sm" class="py-3 " theme="bg-feSecondary-000 dark:bg-feSecondaryDark-grey text-fePrimary-500 dark:text-fePrimaryDark-white"> {{cancelButton}} </ps-button>
                <ps-button @click="actionClicked('yes')" textSize="text-xxs lg:text-sm" class="py-3"  > {{okButton}} </ps-button> -->
                <ps-button @click="actionClicked('yes')" textSize="font-normal text-xxs lg:text-sm" colors="bg-fePrimary-500 cursor-pointer text-feTextLight" rounded="rounded"> {{ okButton }}</ps-button>
                <ps-button @click="actionClicked('no')" textSize="font-normal text-xxs lg:text-sm" colors="dark:bg-gray-800 dark:text-gray-200 hover:text-white" focus="focus:text-white focus:bg-fePrimary-500" border="border border-gray-300 dark:border-gray-500" rounded="rounded"> {{ cancelButton }} </ps-button>
            </div>
        </template>
    </ps-modal>
</template>
