...
<Head>
  <link rel="icon" type="image/svg+xml" :href="$page.props.thumb1xUrl + '/'+ appInfoStore.appInfo.data?.frontendConfigSetting?.frontendIcon?.imgPath" />
    ...
    <div v-if="dataReady" :dir="getDir()" class='bg-feBackground h-full'>
    <div  class="  w-full h-full flex flex-col min-h-screen ">

        <!-- Body -->
        <main class="flex-grow bg-feBackground dark:bg-backgroundDark">
            <slot />
        </main>
        <!-- End Body -->
...
export default defineComponent({
  ...
  props:['authUser','backendSetting','firebaseConfig','webPushKey'],
    setup(props) {
