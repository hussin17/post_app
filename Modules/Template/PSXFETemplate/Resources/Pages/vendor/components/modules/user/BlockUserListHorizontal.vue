    ...
    <div class="shadow-sm relative bg-feSecondary-50 dark:bg-feSecondary-800 rounded-lg flex flex-col gap-4 p-4">
        <div class="flex gap-4">
            <ps-route-link :to="{ name: 'fe_other_profile', params: { userId: block.userId }}">
                <div class="w-20 h-20 relative cursor-pointer">
                    ...
                    <div v-if="block.isVerifybluemark == '1'" class="w-8 h-8 bg-blue-500 rounded-full p-1 absolute bottom-0 right-0">
            ...
            <div class="grow flex flex-col gap-2 justify-center truncate">
                <ps-route-link :to="{ name: 'fe_other_profile', params: { userId: block.userId }}" class="cursor-pointer">
                    <ps-label textColor="text-lg font-semibold text-feSecondary-800 dark:text-feSecondary-000">{{ block.userName.length > 17 ? block.userName.slice(0,17)+"..." : block.userName }}</ps-label>
                </ps-route-link>
                ...
                <div class="flex flex-col  ">
                    <div>
                        <router-link v-if="block.isShowPhone == '1'" class="cursor-pointer">
                                ...
                                <ps-label textColor="text-sm font-normal text-feSecondary-800 dark:text-feSecondary-000">{{ block.userPhone }}</ps-label>
                    ...
                    <div>
                        <router-link v-if="block.isShowEmail == '1'" class="cursor-pointer">
                            ...
                            <ps-label textColor="text-sm font-normal text-feSecondary-800 dark:text-feSecondary-000">{{ block.userEmail }}</ps-label>
                        </router-link>
                    </div>
                </div>
            </div>
        ...
        <div class="grid grid-cols-2">
            <div class="flex items-center gap-1">
                ...
                <ps-label textColor="text-sm font-medium text-feSecondary-800 dark:text-feSecondary-000">{{user ? block.followerCount:'0'}} {{ $t("profile__followers")}}</ps-label>
            </div>
            <div class="flex items-center gap-1">
                <ps-icon class="text-feSecondary-800 dark:text-feSecondary-000" name="shoppingCart-fill" w="24" h="24" viewBox="0 0 24 24"/>
                <ps-label textColor="text-sm font-medium text-feSecondary-800 dark:text-feSecondary-000">{{user ? block.activeItemCount:'0'}} {{ $t("user__item") }}</ps-label>
            </div>
        </div>
