    ...
    <div class="shadow-sm relative bg-feSecondary-50 dark:bg-feSecondary-800 rounded-lg flex flex-col gap-4 p-4">
        <div class="flex gap-4">
            <ps-route-link :to="{ name: 'fe_other_profile', params: { userId: user.userId }}">
                <div class="w-20 h-20 relative cursor-pointer">
                    ...
                    <div v-if="user.isVerifybluemark == '1'" class="w-8 h-8 bg-blue-500 rounded-full p-1 absolute bottom-0 right-0">
            ...
            <div class="grow flex flex-col gap-2 justify-center truncate">
                <ps-route-link :to="{ name: 'fe_other_profile', params: { userId: user.userId }}" class="cursor-pointer">
                    <ps-label textColor="text-lg font-semibold text-feSecondary-800 dark:text-feSecondary-000">{{ user.userName.length > 17 ? user.userName.slice(0,17)+"..." : user.userName }}</ps-label>
                </ps-route-link>
                <ps-route-link :to="{ name: 'fe_review_list',query: { user_id: user.userId } }">
                    <rating :rating="user ? user.overallRating:0" :maxStars="5" iconColor="text-yellow-500" size="md"/>
                </ps-route-link>
        ...
        <div class="grid grid-cols-2">
            <div class="flex items-center gap-1">
                <ps-icon class="text-feSecondary-800 dark:text-feSecondary-000" name="user-group-fill" w="24" h="24" viewBox="0 0 24 24"/>
                <ps-label textColor="text-sm font-medium text-feSecondary-800 dark:text-feSecondary-000">{{user ? user.followerCount:'0'}} {{ $t("profile__followers")}}</ps-label>
            </div>
            <div class="flex items-center gap-1">
                <ps-icon class="text-feSecondary-800 dark:text-feSecondary-000" name="shoppingCart-fill" w="24" h="24" viewBox="0 0 24 24"/>
                <ps-label textColor="text-sm font-medium text-feSecondary-800 dark:text-feSecondary-000">{{user ? user.activeItemCount:'0'}} {{ $t("user__item") }}</ps-label>
            </div>
        </div>

        <div class="h-9" @click="followClick(user)">
            <ps-button v-if="user.isFollowed == '0'" class="absolute bottom-4 right-4 left-4">{{ $t('other_profile__follow')}}</ps-button>
            <ps-button v-else class="absolute bottom-4 right-4 left-4">{{ $t('other_profile__following')}}</ps-button>
        </div>
    </div>
