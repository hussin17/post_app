        ...
        <template #title>
            <div class="flex">
               <div class="grow">
                ...
                <ps-icon name="close" class="text-gray-500 hover:cursor-pointer" @click.prevent="psmodal.toggle(false)"></ps-icon>
            </div>
        </template>
        ...
        <template #body>
            <div>
                ...
                <ps-dropdown align="" :isFixed="true" boxPositioning="rtl:right-1/2 rtl:translate-x-1/2 ltr:left-1/2 ltr:-translate-x-1/2 top-1/2  -translate-y-[9%]" class="mt-3 w-56" >
                    ...
                    <template #filter >
                        <div>
                            <ps-input-with-left-icon rounded="rounded-t-lg" theme="ring-fePrimary-200 dark:ring-fePrimaryDark-grey focus:outline-none focus:ring focus:border-fePrimary-200 border-secondary-300 lg:rounded rounded-x bg-feSecondary-200 dark:bg-fePrimaryDark-black text-feSecondary-500 dark:text-feSecondaryDark-white" height="h-10" @keyup.enter="filterKeywordUpate(itemLocationStore.filterKeyword)"  v-model:value="itemLocationStore.filterKeyword" v-bind:placeholder= "$t('search_city')" style="width: 14.5rem;">
                                <template #icon>
                                    <ps-icon textColor="text-fePrimary-500 dark:text-feSecondaryDark-grey" name="search" class='cursor-pointer' @click="filterKeywordUpate(itemLocationStore.filterKeyword)" />
                                </template>
                    ...
                    <template #list>
                        <div class="pt-2 z-50">
                            ...
                            <div v-else class="w-56 flex py-4 px-2 hover:bg-fePrimary-000 dark:hover:bg-fePrimaryDark-black cursor-pointer items-center"  @click="locationFilterClicked('',$t('item_list__all'),'','')" >
                                <ps-label class="ms-2" :class="locationId =='' ? ' font-bold text-fePrimary-700' : ''"  >{{ $t('item_list__all')}} </ps-label>
                            </div>
                            <div v-for="selectData in itemLocationStore.itemLocationList.data" :key="selectData.id" class="w-56 flex py-4 px-2 hover:bg-fePrimary-000 dark:hover:bg-fePrimaryDark-black cursor-pointer items-center"  @click="locationFilterClicked(selectData.id,selectData.name,selectData.lat,selectData.lng)" >
                ...
                <ps-dropdown v-if="appInfoStore.appInfo.data?.psAppSetting?.isSubLocation == '1'" align="center" :class="`mt-3 w-56 ${locationCity == '' ? 'pointer-events-none' : ''}`" :isFixed="true" boxPositioning="rtl:right-1/2 rtl:translate-x-1/2 ltr:left-1/2 ltr:-translate-x-1/2 top-1/2  translate-y-[21%]"  @click="loadLocationTownshipList">
                    ...
                    <!-- <template #filter>
                        <div>
                            <ps-input-with-left-icon :placeholder="$t('search_location')" rounded="rounded-t-lg" theme="ring-fePrimary-200 dark:ring-fePrimaryDark-grey focus:outline-none focus:ring focus:border-fePrimary-200 border-secondary-300 lg:rounded rounded-x bg-feSecondary-200 dark:bg-fePrimaryDark-black text-feSecondary-500 dark:text-feSecondaryDark-white" height="h-10" style="width: 15rem;" @keyup.enter="filtersubLocationUpdate(itemLocationTownshipStore.filterKeyword)" v-model:value="itemLocationTownshipStore.filterKeyword">
                                <template #icon>
                                    <ps-icon textColor="text-fePrimary-500 dark:text-feSecondaryDark-grey" name="search" class='cursor-pointer' @click="flter(itemCurrencyStore.filterKeyword)"/>
                                </template>
                            </ps-input-with-left-icon>
                        </div>
                    </template> -->
                    <template #list>
                        ...
                        <div v-else class="w-56 flex py-4 px-2 hover:bg-fePrimary-000 dark:hover:bg-fePrimaryDark-black cursor-pointer items-center"  @click="townshipFilterClicked('',$t('item_list__all'),'','')" >
                            <ps-label class="ms-2" :class="townshipId =='' ? ' font-bold text-fePrimary-700' : ''"  > {{$t('item_list__all')}} </ps-label>
                        </div>
                        <div v-for="township in itemLocationTownshipStore.locationTownshipList.data" :key="township.id" class="w-56 flex py-4 px-2 hover:bg-fePrimary-000 dark:hover:bg-fePrimaryDark-black cursor-pointer items-center" @click.prevent="townshipFilterClicked(township.id,township.townshipName)">
                    ...
                    <template #loadmore>
                        <div class="mb-2 w-56">
                            ...
                            <ps-label v-if="!itemLocationTownshipStore.isNoMoreRecord" class="flex mt-4 ms-4 mb-2 underline font-bold cursor-pointer"   @click="loadLocationTownshipList" > {{ $t("search_for_large_screem__load_more") }} </ps-label>
                        </div>
                    </template>
                </ps-dropdown>
...
export default{
    ...
    setup(props,context){
        ...
        function loadLocationTownshipList(){
            itemLocationTownshipStore.loadItemLocationTownshipList();
        }
