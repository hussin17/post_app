<template>
    <div class="cursor-pointer "  @click="gotoDetail(chathistory)">
        ...
        <!-- {{ chathistory }}
        {{ Date.now() }} -->
        <div :class="`h-[80px] sm:h-[100px] rounded-lg shadow-sm overflow-hidden mb-3 ${chathistory.buyerUnreadCount > 0 ? 'bg-gray-200' : ''} dark:bg-feSecondary-800`">
           <div class="flex h-full">
                ...
                <div class="grow px-4 py-1">
                    ...
                    <div class="flex justify-between w-full">
                        <ps-label class="font-normal text-sm text-gray-400" textColor="text-gray-600">{{ chathistory ? chathistory.latestChatMessage : ''}}</ps-label>
                        <span class="flex justify-center items-center font-normal text-sm rounded-full w-5 h-5  bg-fePrimary-600 text-gray-200" v-if="chathistory.buyerUnreadCount > 0">{{chathistory ? chathistory.buyerUnreadCount : '0'}}</span>
