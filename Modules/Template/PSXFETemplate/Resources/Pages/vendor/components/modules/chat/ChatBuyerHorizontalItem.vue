<template>
    <div class="cursor-pointer " @click="gotoDetail(chathistory)">
        ...
        <!-- Pscard -->
        <div :class="`h-[80px] sm:h-[100px] rounded-lg shadow-sm overflow-hidden mb-3 ${chathistory?.sellerUnreadCount > 0 ? 'bg-gray-200' : ''} dark:bg-feSecondary-800`">
           <div class="flex h-full">
                <div class="h-full w-[64px] sm:w-[100px] relative flex items-center">
                    <img alt="Placeholder" class="rounded-full bg-transparent h-16 w-16 sm:h-full sm:w-full border-2 border-gray-300"
                    ...
                    <div class="absolute left-9 top-12 w-6 h-6 block sm:hidden">
                        <img alt="Placeholder"  class="rounded-full w-full h-full border-2 border-gray-100"
                ...
                <div class="grow px-4 py-1">
                    ...
                    <div class="flex justify-between items-center w-full">
                        ...
                        <ps-label class="text-xs">{{ chatTimeHistory(chathistory.addedDate) }}</ps-label>
                    </div>
                    <div class="flex justify-between w-full items-center">
                        <ps-label class="font-normal text-sm text-gray-400 lg:hidden block" textColor="text-gray-600" v-if="chathistory">{{ chathistory.latestChatMessage.length > 20 ? chathistory.latestChatMessage.slice(0,15)+"......" : chathistory.latestChatMessage}}</ps-label>
                        <ps-label class="font-normal text-sm text-gray-400 lg:block hidden" textColor="text-gray-600" v-if="chathistory">{{ chathistory.latestChatMessage }}</ps-label>
                        <span class="flex justify-center items-center font-normal text-sm rounded-full w-5 h-5  bg-fePrimary-600 text-gray-200" v-if="chathistory.sellerUnreadCount > 0">{{chathistory ? chathistory.sellerUnreadCount : '0'}}</span>
                        <div class="w-10 h-10 flex justify-center items-center" v-else>
                            <ps-icon name="done_all" class="text-gray-400" w="24" h="24"></ps-icon>
                        </div>
                    </div>
                </div>
